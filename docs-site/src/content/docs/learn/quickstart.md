...
The installer downloads the matching release archive, verifies its SHA-256 checksum, and installs `shp` plus bundled Tree-sitter parser assets into `~/.local/bin`. Replace `v0.9.0` with the release tag you want to pin.
...
- Pin install and action versions to an explicit tag such as `v0.9.0`
