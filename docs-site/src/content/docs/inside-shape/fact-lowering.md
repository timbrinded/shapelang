...
Lowering never sees the changed-file list. At rule time, `checkCoverage` reads `model.shapeUpdatePaths` and `model.attestations`, and `checkBindings` reads `model.bindings` and `model.attestations`. Neither reads facts, and both apply the rule that a ref or attestation counts only when its declaring `.shape` file is in the changed-file list; see [Rule Evaluation](/shapelang/inside-shape/rule-evaluation/#interactions). The user-facing coverage rules are in [Keep the Model Current](/shapelang/guides/keep-model-current/).
