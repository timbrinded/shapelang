...
- The options. Only five checks read them: `checkFreshness` reads `freshnessDate`; `checkCoverage` and `checkBindings` read `changedFiles`, `repoRoot`, and the base model derived from `baseModules`; `checkStaleAttestations` reads its attestation keys; and `checkCitedPaths` reads `repositoryFiles` and `repoRoot`. `allowUnknownEffects` and `includeFacts` apply after the rules, in `checkLoweredShapeModel`.
...
- **The current-file filter.** With an empty changed-file list, `checkCoverage` and `checkBindings` report nothing. Otherwise `changedFileContext` normalizes the list against `repoRoot`, and a Shape-update ref counts only when `provenanceFileChanged` finds its declaring `.shape` file in that list. An attestation follows the same rule unless `baseModules` is set; then `isCurrentAttestation` counts it only when its `attestationKey` (kind, path, and reason) is absent from the lowered base model. With a base, `checkBindings` also skips a triggering `.shape` file whose text without attestations (`model.attestationFreeTexts`, kept per file and absent for a module built in code) equals the base version's, so a change that only adds or deletes attestations, such as `shp attest prune`, never demands a bound docs change. The incremental checker keys cached results on the base model's attestation keys and attestation-free texts, so a different base re-runs the checks. The attestation, governed-path, and binding rules applied after this filter are in [Keep the Model Current](/shapelang/guides/keep-model-current/).
- **Cited paths.** When the caller passes `repositoryFiles` (`shp check --check-cited-paths`), `checkCitedPaths` collects every function's `source` path and complete-effect `evidence` paths, reading `model.components` directly because `shapeUpdatePaths` leaves out generated AST functions, every candidate effect's `source`, every `evidence` path of rationale, memory, and reevaluation, and every memory `observed` path, and reports each one missing from the list once, with every citing declaration as its cause. Attestation sources are exempt, since attesting a deletion names a removed file. The checker reads no files; the list is part of the incremental checker's cache key.
