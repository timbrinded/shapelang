...
- The options. Only four checks read them: `checkFreshness` reads `freshnessDate`; `checkCoverage` and `checkBindings` read `changedFiles`, `repoRoot`, and the base model derived from `baseModules`; and `checkStaleAttestations` reads its attestation keys. `allowUnknownEffects` and `includeFacts` apply after the rules, in `checkLoweredShapeModel`.
