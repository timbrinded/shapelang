...
Default discovery must include every tracked non-hidden `shape/**/*.shape` file; a sparse
checkout omitting any of these files fails with a usage error naming the missing
path. Hidden files and directories remain excluded from default discovery.
Explicit positional file arguments continue to select only those files.
