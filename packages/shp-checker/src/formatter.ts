...
function formatRelation(relation: RelationDecl): string {
  ...
  for (const member of relation.members) {
    if (isRelationKindDecl(member)) {
      ...
      const sortedRoles = [...member.roles].sort((left, right) => left.name.localeCompare(right.name));
      rolesLines.push(`roles { ${sortedRoles.map((role) => `${role.name} as ${role.role}`).join(", ")} }`);
    } else if (isRelationSummaryDecl(member)) {
      summaryLine = `summary ${quote(member.value)}`;
    }
  }

  const lines = [kindLine, connectsLine, ...rolesLines, summaryLine].filter((line) => line.length > 0);
