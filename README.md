...
- `shp graph [SYMBOL] [--kind KIND] [--stats]`: with a SYMBOL, print the hyperedges incident to that symbol; without a SYMBOL, print the whole hypergraph grouped by kind; with `--stats`, print aggregate vertex, hyperedge, and incidence counts. `--kind` filters in any mode.
